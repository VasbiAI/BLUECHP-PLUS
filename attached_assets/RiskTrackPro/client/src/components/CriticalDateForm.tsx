...
const CriticalDateForm: React.FC<CriticalDateFormProps> = ({
  ...
  useEffect(() => {
    if (initialData?.dueDate) {
      const parsedDate = parseDate(initialData.dueDate);
      if (parsedDate) {
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 mb-4">
                      <div className="space-y-0.5">
                        <FormLabel className="text-base">Business Days Calculation</FormLabel>
                        <FormDescription>
                          Use business days (Monday-Friday) for reminder calculations
                        </FormDescription>
                      </div>
                      <FormControl>
                        <Switch
                ...
                {form.watch('dueDate') && (
                  <Card className="mt-4">
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm font-medium">Reminder Preview</CardTitle>
                      <CardDescription>Based on the current settings</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-2">
                        {calculateReminderDates(
                          form.watch('dueDate'),
                          form.watch('useBusinessDays'),
                          [
                            Number(form.watch('reminder1Days') || 0),
                            Number(form.watch('reminder2Days') || 0),
                            Number(form.watch('reminder3Days') || 0),
                            Number(form.watch('reminder4Days') || 0)
                          ].filter(day => day > 0)
                        ).map((reminder, index) => (
                          <div key={index} className="flex justify-between items-center py-1 border-b">
                            ...
                            <div className="text-sm text-muted-foreground">
                              {reminder.daysRemaining} {form.watch('useBusinessDays') ? 'business' : 'calendar'} days before due date
                            </div>
                          </div>
                        ))}
                        
                        <div className="flex justify-between items-center py-1 border-b font-semibold">
                          <div className="flex items-center">
                            <AlertCircle className="h-4 w-4 mr-2 text-orange-500" />
                            <span>{form.watch('dueDate')}</span>
                          </div>
                        ...
                        {Number(form.watch('postTriggerDateReminderDays') || 0) > 0 && (
                          <div className="flex justify-between items-center py-1">
                            <div className="flex items-center">
                              ...
                              <span>
                                {formatDate(
                                  form.watch('useBusinessDays') 
                                    ? getDateAfterBusinessDays(parseDate(form.watch('dueDate')) || new Date(), Number(form.watch('postTriggerDateReminderDays') || 0))
                                    : getDateAfterCalendarDays(parseDate(form.watch('dueDate')) || new Date(), Number(form.watch('postTriggerDateReminderDays') || 0))
                                )}
                              </span>
                            </div>
                            <div className="text-sm text-muted-foreground">
                              {Number(form.watch('postTriggerDateReminderDays') || 0)} {form.watch('useBusinessDays') ? 'business' : 'calendar'} days after due date
                            </div>
                          </div>
                        )}
                      </div>
